    ...
    init: async ({ client, prisma, kazagumo }) => {
        client.on('interactionCreate', async (interaction) => {
            ...
            if (interaction.options.getSubcommand() == "update" && interaction.options.getFocused() == "track") {
                ...
                let choices = action === "remove" ? tracks.map((track) => {
                    return {
                        name: track.title,
                        value: track.uri
                    }
                ...
                if (choices.length === 0 || action === "add") {
                    const kazTracks = (await kazagumo.search(trackName, { requester: interaction.member as GuildMember })).tracks
    ...
    options: [
        {
            ...
            options: [
                ...
                },
                {
                    ...
                    type: ApplicationCommandOptionType.String,
                    required: true,
                    choices: [{ name: "Public", value: "public" }, { name: "Private", value: "private" }, { name: "Guild", value: 'guild' }]
                },
            ],
        },
        {
    ...
    execute: async ({ interaction, player, handler }) => {
        ...
        switch (subCommand) {
                ...
                try {
                    playlist = await handler.prisma.playlist.create({
                        data: {
                            ...
                            tracks: {
                                connect: await transferTracksTDB([player!.queue.current!, ...player!.queue.map((track) => track)], handler.prisma)
                            }
                        },
                        include: {
                            tracks: true
            ...
            case "update": {
                ...
                const uri = (await handler.kazagumo.search(interaction.options.getString("track", true), { requester: interaction.member as GuildMember })).tracks[0];
