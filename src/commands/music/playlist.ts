...
const getPlaylists = async (
	...
	return (
		...
		if (playlist.userId && !playlist.guildId && playlist.userId !== userId)
			return false;
		return true;
	}) as Playlist[];
};
		...
		switch (subCommand) {
			...
			case "play": {
				...
				if (
					playlist.userId &&
					!playlist.guildId &&
					playlist.userId !== interaction.user.id
				) {
					return {
						content:
							"You don't have permission to play this playlist as it is private",
						ephemeral: true,
					};
				}
				...
				const tracks = await transferTracksFDB(
					playlist.tracks,
					interaction.member,
					handler.kazagumo,
				);
