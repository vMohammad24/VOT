...
export default {
    ...
    execute: async ({ user, args, handler: { prisma } }) => {
        ...
        if (choice !== 'player' && choice !== 'banker' && choice !== 'tie') return {
            content: 'Please provide a valid choice to bet on (Player, Banker, Tie)',
            ephemeral: true
        }
        ...
        const embed = new EmbedBuilder()
            .setTitle("Baccarat")
            .setDescription(`🃏 | Player: ${playerTotal} | Banker: ${bankerTotal} | 🃏\n`)
