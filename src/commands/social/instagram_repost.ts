...
import type { InstagramJsonResponse } from "../../util/instagram";
...
export default {
	...
	execute: async ({ args }) => {
		...
		if (!url.startsWith(requireStartWith))
			return {
				content: "This is not a valid instagram reel url",
				ephemeral: true,
			};
		...
		if (index === -1)
			return { content: "Failed to repost this reel.", ephemeral: true };
		...
		return {
			...
			files: [
				new AttachmentBuilder(media.video_versions[0].url, { name: filename }),
			],
		};
	},
} as ICommand;
