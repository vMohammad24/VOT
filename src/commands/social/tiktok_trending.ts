...
export default {
    ...
    execute: async ({ interaction, message }) => {
        const res = await axios.get('https://www.tiktok.com/api/explore/item_list/?aid=2&browser_language=en-US&count=3&data_collection_enabled=false&history_len=4&region=US&tz_name=America%2FVirgin&user_is_login=true&device_id=1&categoryType=120&language=en', {
            ...
            pages: await Promise.all(videos.map(async (video) => {
                ...
                return {
                    page:
                    {
                        ...
                        files: [new AttachmentBuilder(Buffer.from(videoPlay.data), { name: 'VOT_TT_Trending.mp4' })],
                    },
