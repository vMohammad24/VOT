...
import type { TikTokVideo } from "../../util/tiktok";
...
export default {
	...
	execute: async ({ args }) => {
		...
		if (!url)
			return { ephemeral: true, content: "Please provide a Tiktok video URL" };
		try {
			new URL(url);
		} catch (e) {
			return {
		...
		return {
			...
			embeds: [
				new VOTEmbed()
					...
					.setColor(color)
					.setTimestamp(Number.parseInt(video.createTime) * 1000),
			],
			files: [
