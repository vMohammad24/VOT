...
export default {
	...
	autocomplete: async (int) => {
		...
		if (!action || isNullish(action))
			int.respond([{ name: "Search to find an action", value: "" }]);
		const actions = Object.keys(AuditLogEvent);
		const filtered = actions.filter(
			(a) =>
				a.toLowerCase().includes(action.toLowerCase()) ??
				camelToTitleCase(a).toLowerCase().includes(action.toLowerCase()),
		);
		const options = filtered.map((a) => ({
			name: camelToTitleCase(a),
			value: (AuditLogEvent as Record<string, any>)[a].toString(),
		}));
	...
	execute: async ({ guild, args, interaction, message }) => {
		...
		if (!guild) return;
		const logs = await guild.fetchAuditLogs({ type: Number.parseInt(action) });
		const pages: PaginationPage[] = logs.entries.map((entry) => {
			const r = entry.action
				? camelToTitleCase(AuditLogEvent[entry.action])
				: undefined;
				...
				? entry.changes
						.map((change) => {
							const oldValue =
								...
								change.old?.toString() === "[object Object]"
									? "unknown"
									: change.old;
							const newValue =
								typeof change.new === "object" &&
								change.new?.toString() === "[object Object]"
									? "unknown"
									: change.new;
		...
		if (pages.length === 0)
			return { content: "No logs found", ephemeral: true };
