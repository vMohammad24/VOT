...
export default {
    ...
    execute: async ({ guild, member: moderator, args }) => {
        ...
        if (
            ...
            return {
                content: 'You cannot unmute this member as they have a higher role than you',
                ephemeral: true,
            };
        }
        ...
        const newCase = await createCase(guild.id, 'TimeoutRemove', member.id, moderator.id, reason);

        await member.timeout(null, `${reason || 'No reason provided'} (Case #${newCase.caseId})`);
