...
export default {
    ...
    execute: async ({ args, guild, member }) => {
        ...
        if (member.roles.highest.position <= user.roles.highest.position) return { content: "You do not have permission to softban this user", ephemeral: true };
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor("Red")
            .setTimestamp()
            .setFooter({ text: `Softbanned by ${member.user.tag}`, iconURL: member.user.displayAvatarURL() })
        return { embeds: [embed] };
    }
} as ICommand