...
export default {
    ...
    execute: async ({ args, handler }) => {
        ...
        if (!cmdName) return { ephemeral: true, content: 'Please provide a command to toggle' }
        const cmd = handler.commands!.find(c => c.name?.toLowerCase() == cmdName.toLowerCase());
        if (!cmd) return { ephemeral: true, content: 'Command not found' }
        cmd.disabled = !cmd.disabled;
        return { ephemeral: true, content: `${cmd.name} has been ${cmd.disabled ? 'disabled' : 'enabled'}` }
    }
} as ICommand