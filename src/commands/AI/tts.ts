...
const voices = [
	...
	{
		name: "Stormtrooper (Star Wars) (Characters)",
		value: "en_us_stormtrooper",
	},
	{
		name: "Rocket (Guardians of the Galaxy) (Characters)",
		value: "en_us_rocket",
	},
...
export default {
	...
	autocomplete: async (inter) => {
		...
		if (!input)
			return inter.respond(
				voices.map((v) => ({ name: v.name, value: v.value })).slice(0, 25),
			);
		inter.respond(
			fuzzySearch(input, voices).map((v) => ({ name: v.name, value: v.value })),
		);
	},
	// userTier: 'Premium',
	// slashOnly: true,
	execute: async ({ args, interaction, handler }) => {
		...
		if (!text)
			return { ephemeral: true, content: "Please provide text to convert." };

		const matchedVoices = fuzzySearch(voice, voices);
		const selectedVoice =
			matchedVoices.length > 0 ? matchedVoices[0].value : "en_us_001";
		...
		if (!data.success) {
			return {
				ephemeral: true,
				content: data.error || "An error occurred while generating the audio.",
			};
		}
