...
import { type AIMessage, advancedChat } from "../../util/ai";
...
export default {
	...
	execute: async ({
		...
		if (!question) {
			...
			return {
				content:
					"Please supply a quesiton to ask the AI, it has context but it can NOT read your mind. (YET)",
				ephemeral: true,
			};
		}
		const shouldSearch =
		...
		async function fetchChannelMessages(channel: GuildTextBasedChannel) {
			const messagesArray =
				channel.messages.cache.size < 50
					? Array.from(
			...
			const pinnedMessages = Array.from(
		...
		async function messageToText(m: Message<boolean>): Promise<string> {
			...
			if (m.hasThread && m.thread) {
				...
				if (threadMessages.size > 0) {
					const threadTexts = await Promise.all(
						Array.from(threadMessages.values()).map(messageToText),
					);
		...
		const channelMessage = channelMessages
			? (
					await Promise.all(
						channelMessages.map(
							async (m: Message<boolean>) => await messageToText(m),
						),
					)
				).join("\n")
			: "";
		const users = guild
			? await guild.members.cache
					.map(
						(user) =>
							`Display name: ${user.displayName} (ID: ${user.id}) - Role: ${user.roles.highest.name} - Joined: ${user.joinedAt} - Boosting since ${user.premiumSinceTimestamp}`,
					)
					.join("\n")
			: undefined;
        ...
        `,
			},
			{
				role: "assistant",
				content:
					"I understand. I will provide accurate and concise responses using the command information you provided while following the proper Discord formatting conventions.",
			},
			...previousMessages,
		];
