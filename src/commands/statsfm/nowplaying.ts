...
export default {
    ...
    execute: async ({ user }) => {
        ...
        if (!np || !np.item || !np.item.track) return {
            content: 'Nothing is currently playing',
            ephemeral: true
        };

        const [previewURL, fileEx] = (() => {
            if (np.item.track.appleMusicPreview) return [np.item.track.appleMusicPreview, 'm4a'];
            if (np.item.track.spotifyPreview) return [np.item.track.spotifyPreview, 'mp3'];
            return [];
        })();
        const embed = await new VOTEmbed()
            ...
            .dominant()
        const features = np.item.track.externalIds.spotify[0] ? await getSpotifyFeatures(np.item.track.externalIds.spotify[0]) : null;
        if (features) {
            const fP = (v: number) => numeral(v).format('0%')
            embed.setFields([
                { name: 'Acousticness', value: fP(features.acousticness), inline: true },
                { name: 'Danceability', value: fP(features.danceability), inline: true },
        ...
        if (np.item.progressMs != 0) {
            ...
            embed.setDescription(`Progress: ${numeral(np.item.progressMs / np.item.track.durationMs).format('0%')} (${formatTime(np.item.progressMs)}/${formatTime(np.item.track.durationMs)})`);
        }
        return {
            embeds: [
                embed
