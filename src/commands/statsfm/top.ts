...
import { getUser, getUserTopArtists, getUserTopGenres, getUserTopTracks } from "../../util/statsfm";
                ...
                return {
                    embeds: [
                        await new VOTEmbed()
                            ...
                            .setDescription(
                                genres.items.slice(0, 10).sort((a, b) => a.position - b.position).map((g, i) => `${i + 1}. ${g.genre.tag} - ${g.artistCount ?? '??'} artist${g.artistCount == 1 ? '' : 's'}`.trim()).join('\n')
                            )
                            .dominant()
                    ]
                }
                ...
                return {
                    embeds: [
                        await new VOTEmbed()
                            .setThumbnail(fmUser.item.image)
                            .setDescription(
                                artists.items.slice(0, 10).sort((a, b) => a.position - b.position).map((a, i) => `${i + 1}. [${a.artist.name}](${`https://open.spotify.com/artist/${a.artist.externalIds.spotify}`})`.trim()).join('\n')
                            )
                            .dominant()
                    ]
                }
