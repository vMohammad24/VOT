...
export default {
	...
	execute: async ({ user, args }) => {
		const username =
			...
			(await getUserByID(user.id, { statsfmUser: true })).statsfmUser;
		...
		const embed = await new VOTEmbed()
			.setAuthor({
				name: u.item.customId ?? u.item.displayName,
				iconURL: u.item.image,
			})
			.setTitle(u.item.displayName)
			.setURL(`https://stats.fm/user/${u.item.customId || u.item.id}`)
			.setThumbnail(u.item.image)
			.dominant();
		if (u.item.isPlus && u.item.isPro)
			embed.addDescription("This user has stats.fm Plus and Pro");
		...
		if (u.item.spotifyAuth) {
			actionRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
				new ButtonBuilder()
					...
					.setLabel("Spotify")
					.setURL(
						`https://open.spotify.com/user/${u.item.spotifyAuth.platformUserId}`,
					)
					.setEmoji(getEmoji("spotify").toString()),
			);
		}
		return {
			embeds: [embed],
