	...
	execute: async ({ args, interaction }) => {
		...
		try {
			...
			const embed = new VOTEmbed()
				.setAuthor({ name: `${config.name} Transaction Details`, iconURL: config.icon })
				.addFields(
					{ name: '**Block Height**', value: `\`${tx.block_height.toString()}\``, inline: true },
					{ name: '**Total Amount**', value: `\`${tx.total / 100000000} ${crypto.toUpperCase()}\``, inline: true },
					{ name: '**Fees**', value: `\`${tx.fees / 100000000} ${crypto.toUpperCase()}\``, inline: true },
					{ name: '**Received**', value: `\`${new Date(tx.received).toLocaleString()}\``, inline: true },
					{ name: '**Confirmed**', value: `\`${new Date(tx.confirmed).toLocaleString()}\``, inline: true },
					{ name: '**From**', value: tx.inputs[0].addresses.map(addr => `\`${addr}\``).join('\n'), inline: false },
					{
						name: '**To**',
						value: tx.outputs
							.map(out => `\`${out.addresses[0]}\` (\`${out.value / 100000000} ${crypto.toUpperCase()}\`)`)
							.join('\n'),
						inline: false
					}
				)
			...
			return {
				content: 'Error fetching transaction details. Please check the transaction ID and try again.',
				ephemeral: true,
			};
		}
	},
