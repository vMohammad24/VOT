...
export default {
	...
	autocomplete: async (inter) => {
		...
		if (!input)
			return inter.respond([
				{ value: "", name: "Please provide a valid username or uuid" },
			]);
		if (inter.responded) return;
	...
	execute: async ({ args }) => {
		...
		if (typeof craftyProfile === "string")
			return {
		...
		const activeSkin =
			labyProfile?.textures.SKIN?.find((s) => s.active)?.image_hash ??
		...
		const embed = new VOTEmbed()
			.setAuthor({
				name: data.username,
				iconURL: head ? "attachment://head.png" : undefined,
			})
		...
		if (data.usernames?.length > 0) {
			const allChanges = data.usernames
				.map((u) => ({
					name: u.username,
