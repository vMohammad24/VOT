...
function getWeatherEmoji(condition: string): string {
    ...
    if (conditionLower.includes('snow') || conditionLower.includes('flurries')) return '❄️';
...
export default {
    ...
    execute: async ({ args, interaction, message }) => {
        ...
        const currentWeatherEmbed = new EmbedBuilder()
            ...
            .setDescription(`**${currentEmoji} ${camelToTitleCase(weather.currentWeather.conditionCode)}**`)
            .setColor(getTemperatureColor(currentTemp))
            .addFields(
                { name: '🌡️ Temperature', value: `${currentTemp}°C (Feels like: ${weather.currentWeather.temperatureApparent}°C)`, inline: false },
                { name: '💧 Humidity', value: `${weather.currentWeather.humidity}%`, inline: true },
                { name: '💨 Wind', value: `${weather.currentWeather.windSpeed} m/s from ${formatWindDirection(weather.currentWeather.windDirection)}`, inline: true },
            )
        ...
        weather.forecastDaily.days.forEach(day => {
            ...
            const dailyForecastEmbed = new EmbedBuilder()
                ...
                .addFields(
                    { name: '🌡️ Temperature Range', value: `${day.temperatureMin}°C to ${day.temperatureMax}°C`, inline: false },
                    {
                        name: `☀️ Daytime ${getWeatherEmoji(day.daytimeForecast.conditionCode)}`, value:
                    ...
                    {
                        name: `🌙 Overnight ${getWeatherEmoji(day.overnightForecast.conditionCode)}`, value:
