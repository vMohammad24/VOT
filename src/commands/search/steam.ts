...
async function searchSteamDB(query: string) {
	const apiKey =
		"MzQ1ZjdkNzVjM2I0YjFkYmFlZDViYTgwNDY5MjNlNGY4NzY1NDEzNmIzMmY3YzIzZjVjYmQ2NjJlMzMxN2YxM3ZhbGlkVW50aWw9MTczMzU5MzI0MiZ1c2VyVG9rZW49YTQzOThiNGZiYzQwOTc1NDkzMGRhOTkyNDU3MjI4YTk=";
	const response = await axios.request({
		method: "POST",
		url: "https://94he6yatei-dsn.algolia.net/1/indexes/*/queries",
		params: {
			"x-algolia-agent":
				"Algolia for JavaScript (5.14.0); Lite (5.14.0); Browser; instantsearch.js (4.75.5); JS Helper (3.22.5)",
...
async function getGameInfo(appId: number) {
	...
	return {
		...
		short_description: isNullish(data.short_description)
			? null
			: turndown.turndown(data.short_description),
		...
		platforms: Object.keys(data.platforms).filter(
			(platform) =>
				data.platforms[platform as keyof typeof data.platforms] === true,
		),
		...
		achievements: data.achievements
			? {
					...
					highlighted: data.achievements.highlighted.map(
						(achievement: any) => ({
...
export default {
	...
	autocomplete: async (interaction) => {
		...
		if (!game)
			return interaction.respond([
				{ name: "Please provide a game to search for", value: "" },
			]);
		const response = await searchGames(game);
		if (!response)
			return interaction.respond([{ name: "No games found", value: "" }]);
		await interaction.respond(
			response.map((hit) => ({ name: hit.name, value: hit.appid })),
		);
	},
	execute: async ({ args, channel }) => {
		...
		const embed = await new VOTEmbed()
			.setTitle(game.name ?? "N/A")
			.setDescription(game.short_description ?? "N/A")
			.addFields(
				{
					name: "Price",
					value: game.price?.discount_percent
						? `~~${game.price.initial_formatted}~~ → ${game.price.final_formatted}`
						: (game.price?.final_formatted ?? "N/A"),
					inline: true,
				},
				{
					name: "Achievements",
					value: game.achievements ? game.achievements.total.toString() : "N/A",
					inline: true,
				},
				{
					name: "Recommendations",
					value: game.recommendations
						? numeral(game.recommendations.total).format("0,0")
						: "N/A",
					inline: true,
				},
				{
					name: "Developers",
					value: game.developers.join(", ") ?? "N/A",
					inline: true,
				},
				{
					name: "Publishers",
					value: game.publishers.join(", ") ?? "N/A",
					inline: true,
				},
				{
					name: "Genres",
					value: game.genres.join(", ") ?? "N/A",
					inline: true,
				},
				{
					name: "Rating",
					value: game.rating ? `${game.rating.rating}+` : "N/A",
					inline: true,
				},
				{
					name: "Platforms",
