	...
	server.ws('/:id/verify', {
		async open(ws) {
			...
			if (aGuild.members.me && !aGuild.members.me.roles.highest.comparePositionTo(guild.VerificationSettings.roleId)) {
				ws.send(
					JSON.stringify({
						error: 'I do not have the permission to give you the verification role. Please contact the server owner.',
					}),
				);
				ws.close();
				return;
						...
						const msg = await channel.send({
							...
							components: [
								new ActionRowBuilder<ButtonBuilder>().addComponents(
									new ButtonBuilder().setCustomId('verify').setLabel('Verify Anyways').setStyle(ButtonStyle.Success),
								),
							],
						});
