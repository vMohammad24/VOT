...
export async function renderSkin3D(input: string, shadow = true) {
	try {
		return Buffer.from(
			await getCachedData(`skin3d:${input}:${shadow}`, 3600, async () => {
				const res = await axios.get(
					`https://skin.laby.net/api/render/skin/${input}.png?shadow=${shadow}`,
					{
...
export async function searchNames(name: string) {
	return getCachedData(`search:${name}`, 300, async () => {
		const res = await axios.get<{
			results: { name: string; uuid: string; user_name: string }[];
...
export async function getHead(input: string) {
	...
	if (!uuid) return null;
	return Buffer.from(
		await getCachedData(`head:${uuid}`, 3600, async () => {
			const res = await axios.get(
				`https://laby.net/texture/profile/head/${uuid}.png?size=8`,
				{
					responseType: "arraybuffer",
				},
			);
...
export async function getUuidFromUsername(
	username: string,
): Promise<UsernameResponse | null> {
	return getCachedData(`uuid:${username}`, 21600, async () => {
		const res = await axios.get(
			`https://laby.net/api/v3/user/${username}/uniqueId`,
		);
...
export async function getOnlineStatus(input: string) {
	...
	return getCachedData(`online:${uuid}`, 60, async () => {
		const res = await axios.get(
			`https://laby.net/api/v3/user/${uuid}/online-status`,
		);
		return res.data;
	});
}
...
export async function getGameStats(input: string) {
	...
	return getCachedData(`stats:${uuid}`, 1800, async () => {
		const res = await axios.get(
			`https://laby.net/api/v3/user/${uuid}/game-stats`,
		);
		return res.data;
	});
}
...
export async function searchSkins(params: {
	...
	return getCachedData(key, 300, async () => {
		const res = await axios.get(
			"https://laby.net/api/v3/search/textures/skin",
			{ params },
		);
		return res.data.results;
	});
...
export async function getCraftyProfile(
	input: string,
): Promise<CraftyProfileResponse | string> {
