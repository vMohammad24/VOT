...
export async function getTikTokTrending(count = 3) {
	try {
		...
		const content = await page
			.$("pre")
			.then((e) => e?.evaluate((node) => node.textContent));
		await page.close();
...
export async function getTikTokVideoPlayAddr(video: Video) {
	return (
		await client.get(video.playAddr, {
			jar,
...
export async function getRelatedVideos(itemId: string, count = 12) {
