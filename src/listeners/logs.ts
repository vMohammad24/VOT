...
const getLogChannel = async (guild: Guild) => {
	...
	return (
		(guild.channels.cache.get(g.loggingChannel) as GuildTextBasedChannel) ||
		null
	);
};
	...
	execute: ({ client, kazagumo }) => {
				...
				components: messageBefore
					? [
							new ActionRowBuilder<ButtonBuilder>().addComponents([
								new ButtonBuilder()
									.setLabel("Jump to Surrounding")
									.setStyle(ButtonStyle.Link)
									.setEmoji("🔍")
									.setURL(messageBefore.url),
							]),
						]
					: [],
		...
		client.on("messageDeleteBulk", async (messages) => {
			...
			const messageBefore = (
				await messages
					.find((m) => m)
					?.channel.messages.fetch({ before: messages.first()?.id, limit: 1 })
			)?.first();
			logChannel.send({
				embeds: [embed],
				files: [{ attachment: Buffer.from(file), name: "bulk-delete.txt" }],
				components: messageBefore
					? [
							new ActionRowBuilder<ButtonBuilder>().addComponents([
								new ButtonBuilder()
									.setLabel("Jump to Surronding")
									.setStyle(ButtonStyle.Link)
									.setEmoji("🔍")
									.setURL(messageBefore.url),
							]),
