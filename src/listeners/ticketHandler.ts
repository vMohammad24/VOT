	...
	execute: async ({ client }) => {
		client.on(Events.InteractionCreate, async (interaction) => {
			...
			if (interaction.customId === 'create_ticket') {
				...
				if (tick.error) {
					const embed = new EmbedBuilder().setTitle('Error').setDescription(tick.error).setColor('Red');
				...
				await interaction.editReply({ content: 'Close request has been cancelled' });
			}
		});
	},
} as IListener;
